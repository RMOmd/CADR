...
def main():
    ...
    if api_key == "b54bcf4d-1bca-4e8e-9a24-22ff2c3d462c":
    ...
    signals = detect_divergences(pairs_data, threshold=1.5, lookback=30)
    ...
    if not signals:
        ...
        if not api_key:
            ...
            signals = [DivergenceSignal(
                ...
                conviction_score=4, metadata={"forced": True}
            )]
        else:
            return
    ...
    # We fetch the data again (or reuse if we saved the dataframes)
    df_a = client.get_historical_ohlcv(asset_a, days=90)
    df_b = client.get_historical_ohlcv(asset_b, days=90)
